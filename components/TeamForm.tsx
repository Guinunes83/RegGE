...
import { DROPDOWN_OPTIONS, DELEGATION_ROLES } from '../constants';
...
export const TeamForm: React.FC<TeamFormProps> = ({ member, mode, onSave, onCancel, onEdit, isReadOnly = false, currentUser }) => {
  ...
  const combinedStudyRoles = React.useMemo(() => {
    const roles: { index?: number, studyId: string, role: string, source: 'team' | 'study', studyName: string }[] = [];
    
    // Configured via TeamForm manully
    if (formData.studyRoles) {
      formData.studyRoles.forEach((sr, idx) => {
        const studyName = activeStudiesFull.find(s => s.id === sr.studyId)?.name || sr.studyId;
        roles.push({ index: idx, studyId: sr.studyId, role: sr.role, source: 'team', studyName });
      });
    }

    // Pre-assigned via StudyForm
    if (formData.id && formData.name) {
      activeStudiesFull.forEach(study => {
        // Is PI?
        if (study.pi === formData.name) {
          if (!roles.some(r => r.studyId === study.id && r.role === 'Investigador Principal')) {
             roles.push({ studyId: study.id, role: 'Investigador Principal', source: 'study', studyName: study.name });
          }
        }
        // Is SI?
    ...
    // Sort alphabetically
    return roles.sort((a,b) => a.studyName.localeCompare(b.studyName));
  }, [formData.studyRoles, formData.id, formData.name, activeStudiesFull]);

  const [selectedStudyId, setSelectedStudyId] = React.useState('');
  const [selectedStudyRole, setSelectedStudyRole] = React.useState('');
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-[2fr,1fr,1fr] gap-3">
            ...
            <TeamInput 
              label="F / M" 
              value={formData.sex} 
              onChange={(v: string) => handleFieldChange('sex', v)} 
              options={['F', 'M']} 
              isView={isView} 
            />
            <TeamInput 
                ...
                <button 
                  onClick={() => {
                    if (selectedStudyId && selectedStudyRole) {
                      const exists = combinedStudyRoles.some(s => s.studyId === selectedStudyId && s.role === selectedStudyRole);
                      if (exists) {
                        showValidation("Este membro já está vinculado a este estudo com esta função.");
                        return;
                      }
                      setFormData({ ...formData, studyRoles: [...(formData.studyRoles || []), { studyId: selectedStudyId, role: selectedStudyRole }] });
                      setSelectedStudyId('');
                      setSelectedStudyRole('');
                    }
                  }}
                  disabled={!selectedStudyId || !selectedStudyRole}
              ...
              <table className="w-full text-left text-sm">
                <thead className="bg-[#007b63] text-white">
                  <tr>
                    ...
                    <th className="px-4 py-2">Função no Estudo</th>
                    {!isView && !isReadOnly && <th className="px-4 py-2 text-right">Ação</th>}
                  </tr>
                </thead>
                <tbody className="divide-y">
                  {combinedStudyRoles.map((sRole, index) => {
                    return (
                        ...
                        {!isView && !isReadOnly && (
                          <td className="px-4 py-3 text-right">
                            {sRole.source === 'team' ? (
                              <button 
                                onClick={() => {
                                  setFormData({ ...formData, studyRoles: (formData.studyRoles || []).filter((_, i) => i !== sRole.index) });
                                }}
                              ...
                              <span className="text-gray-400 font-bold text-[10px] uppercase cursor-help" title="Vá até a tela do estudo para remover ou alterar este vínculo">Fixo</span>
                            )}
                          </td>
                        )}
                      </tr>
                  ...
                  {combinedStudyRoles.length === 0 && (
                    <tr><td colSpan={3} className="px-4 py-4 text-center text-gray-500 text-xs italic">Nenhum estudo vinculado a este membro.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
